...
func update_camera() -> void:
	if (camera_override == null):
		...
		if player and player.has_node("%Camera3D"):
			_main_camera = player.get_node("%Camera3D")
		else:
			_main_camera = get_node(camera_path)
	else:
		_main_camera = camera_override;

## PRIVATE method to get the main camera, returns false if not found
func _get_main_camera() -> bool:
	update_camera()
